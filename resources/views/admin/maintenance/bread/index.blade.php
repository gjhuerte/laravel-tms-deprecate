...
<div class="row">
    <div class="col-sm-12 my-1">
        <h1 class="display-4">{{ __('Maintenance') }}: {{ $args->title }}</h1>
	  </div>
    <div class="col-sm-12 my-1">
        ...
        <table 
            ...
            data-selectable-items="{{ implode(',', $args->selectableColumns) }}"
            {{ isset($args->hasViewButton) ? 'data-show-view-button=""': '' }}
            {{ isset($args->hasEditButton) ? 'data-show-edit-button=""': '' }}
            {{ isset($args->hasRemoveButton) ? 'data-show-remove-button=""': '' }}
            data-confirmation-title="Are you sure?"
            data-confirmation-message="You won't be able to revert this!">

            <thead>
                <td>{{ implode('</td><td>', $args->selectableColumns) }}</td>
                <td></td>
            </thead>
        </table>
	  </div>
    ...
    $(document).ready(function() {
        ...
        var dataTable = table.DataTable( {
            ...
            columns: [
                ...
                { data: function(dataResponse) {
                    return buttonsForDatatables.displayAll(dataResponse);
                } },
            ],
        });
        ...
        buttonsForDatatables = {
            displayAll: function (callback) {
                ...
                return 
                    buttonsForDatatables.view(viewUrl, showViewButton) + 
                    buttonsForDatatables.edit(editUrl, showEditButton) + 
                    buttonsForDatatables.remove(removeUrl, showRemoveButton);
            },

            // Button for viewing a certain resource
            // This will redirect to the page of the
            // Specific resource
            view: function(url, isHidden) {

                // check if the button is hidden from the user
                // if the button is hidden, return no value
                if(isHidden) {
                    return;
                }
        ...
        table.on('click', '.btn-remove', function() {
            ...
            alert.confirmation(confirmationTitle, confirmationMessage, function(result) {
